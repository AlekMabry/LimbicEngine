...
void hud::loadFont()
{
	...
	args.
	FT_Open_Face(ft,)
	int error = FT_New_Face(ft, "fonts/arial.ttf", 0, &face);
	if (error == FT_Err_Unknown_File_Format)
	{ 
		std::cout << "ERROR::FREETYPE: Failed to load font - can be open and read, but unsupported" << std::endl;
	}
	else
	{
		std::cout << "ERROR::FREETYPE: Failed to load font - can not be open or read" << std::endl;
	}
...
void hud::renderText(std::string text, GLfloat x, GLfloat y, GLfloat scale, glm::vec3 color)
{
	...
	glUniform3f(glGetUniformLocation(shd_font_singlecolor, "textColor"), color.x, color.y, color.z);
